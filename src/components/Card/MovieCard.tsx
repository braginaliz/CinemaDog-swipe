...
const StarRating = memo(function StarRating({ value }: { value: number }) {
...
const CastGrid = memo(function CastGrid({ casts }: { casts: Movie["casts"] }) {
  return (
    <div className="overflow-y-auto pr-1 max-h-52 sm:max-h-64 scrollbar-thin scrollbar-thumb-pink-200 scrollbar-track-transparent hover:scrollbar-thumb-pink-300">
      <div className="grid gap-y-3 grid-cols-3 gap-x-1.5 sm:grid-cols-4 sm:gap-x-2 pt-1">
        {casts.map((cast, index) => (
          <div
            key={`${cast.name}-${index}`}
            className="flex flex-col items-center gap-1 sm:gap-1.5"
          >
            <div className="rounded-full overflow-hidden border-2 border-pink-100 bg-pink-50 flex items-center justify-center flex-shrink-0 w-11 h-11 sm:w-14 sm:h-14">
              {cast.profile_path ? (
                <img
...
export const MovieCard = memo(function MovieCard({ movie }: MovieCardProps) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        castRef.current &&
        !castRef.current.contains(e.target as Node) &&
        buttonRef.current &&
      ...
      <div className="relative w-full flex-shrink-0 rounded-t-2xl overflow-hidden bg-pink-50 h-60 xs:h-64 sm:h-72 md:h-80 lg:h-88 xl:h-96 2xl:h-[26rem]">
        {posterUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-pink-200">
            <div className="w-8 h-8 sm:w-10 sm:h-10 opacity-40">
              <FilmIcon />
            </div>
            <span className="text-[10px] sm:text-xs text-pink-300">
              No Poster
            </span>
          </div>
        )}
        ...
        <div className="absolute top-2 left-2 right-2 flex items-center justify-between">
          ...
          <span className="flex items-center gap-1 bg-black/50 backdrop-blur-sm text-yellow-400 font-bold rounded-full text-[10px] px-1.5 py-0.5 sm:text-xs sm:px-2">
            <StarBadgeIcon />
        ...
        {movie.casts && movie.casts.length > 0 && (
          <div className="relative mt-auto">
            <button
              ref={buttonRef}
              onClick={toggleCast}
              className="flex items-center justify-between w-full bg-gradient-to-r from-pink-50 to-rose-50 hover:from-pink-100 hover:to-rose-100 border border-pink-100 hover:border-pink-200 rounded-xl transition-all duration-200 text-pink-500 hover:text-pink-700 font-bold px-2 py-1.5 text-xs sm:px-3 sm:py-2 sm:text-sm"
            >
              ...
              <span className="flex items-center gap-1 sm:gap-1.5">
                <span className="font-semibold bg-pink-100 text-pink-400 rounded-full text-[10px] px-1 py-0.5 sm:text-xs sm:px-1.5">
                  {movie.casts.length}
                </span>
            ...
            {castOpen && (
              <div
                ref={castRef}
                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 bg-white/95 backdrop-blur-md border border-pink-100 rounded-2xl shadow-2xl shadow-pink-200/60 flex flex-col w-56 p-2 xs:w-64 sm:w-72 sm:p-3"
              >
